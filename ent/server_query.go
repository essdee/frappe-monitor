...
// QueryLogCursors chains the current query on the "log_cursors" edge.
func (_q *ServerQuery) QueryLogCursors() *LogCursorQuery {
	query := (&LogCursorClient{config: _q.config}).Query()
...
// WithLogCursors tells the query-builder to eager-load the nodes that are connected to
// the "log_cursors" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ServerQuery) WithLogCursors(opts ...func(*LogCursorQuery)) *ServerQuery {
	query := (&LogCursorClient{config: _q.config}).Query()
...
func (_q *ServerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Server, error) {
	...
	if query := _q.withLogCursors; query != nil {
...
func (_q *ServerQuery) loadLogCursors(ctx context.Context, query *LogCursorQuery, nodes []*Server, init func(*Server), assign func(*Server, *LogCursor)) error {
	...
	query.withFKs = true
	query.Where(predicate.LogCursor(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(server.LogCursorsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.server_log_cursors
		if fk == nil {
			return fmt.Errorf(`foreign-key "server_log_cursors" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
